# ==========================================================
# IKBR SCALPING BOT | VERSION v1.6.0 P196 | STAGE: ACC
# ==========================================================
...
class ScalpingBot:
    ...
    def get_stale_bot_owned_orders_for_trade(
        ...
        def bot_owned_order_match(order, status=None, contract=None):
            ...
            if order_ref and trade_id and str(order_ref).startswith(f"{trade_id}|"):
        ...
        result = {
            ...
            "scan_scope": (
                ...
                "ownership=order_id|perm_id|order_to_trade|orderRef_trade_id"
            ),
        }
        logger.critical(
        ...
        if cancel_failed or remaining_stale_orders:
            with self.trade_analysis_lock:
                ...
                if record is not None:
                    ...
                    record["state"] = "BROKER_ACK_PENDING"
                    record["closed"] = False
                    record["exit_reason"] = PROTECTIVE_EMERGENCY_EXIT_REASON
            logger.critical(
    ...
    def build_finalize_trade_snapshot_if_ready(self, trade_id):
        with self.trade_analysis_lock:
            ...
            if self.is_exit_fully_filled(record) and record["exit_fill_price"] is not None:
                ...
                if record.get("exit_reason") != PROTECTIVE_EMERGENCY_EXIT_REASON:
                    record["exit_reason"] = self.derive_completed_exit_reason(record)
                ready = record["exit_reason"] in ("TP", "SL", "MIXED_EXIT", PROTECTIVE_EMERGENCY_EXIT_REASON)
            elif record["entry_filled"] and record["exit_fill_price"] is not None and record["exit_reason"] in ("TP", "SL", "MIXED_EXIT", PROTECTIVE_EMERGENCY_EXIT_REASON):
                ready = True
            elif (
                record.get("closed")
                and record.get("exit_reason") == PROTECTIVE_EMERGENCY_EXIT_REASON
        ...
        try:
            ...
            if order_id == record["parent_order_id"]:
                ...
                with self.trade_analysis_lock:
                    ...
                    self.append_trade_event(
                        trade_id,
                        f"PROTECTIVE EMERGENCY FLATTEN FILL quantity={realized_quantity} "
    ...
    def execution_worker(self):
        ...
        while True:
            ...
            try:
                ...
                with self.execution_lock:
                    if self.is_session_close_system_job(job):
                        self.run_session_close_sweep(
                            job.get("reason_label") or "session_close_system_job",
