# ==========================================================
# IKBR SCALPING BOT | VERSION v1.6.0 P169 | STAGE: PAPER
# ==========================================================
...
BOT_VERSION = "v1.6.0"
BOT_PATCH = "P169"
BOT_STAGE = "PAPER"  # TEST | PAPER | LIVE
...
class ScalpingBot:
    ...
    def refresh_paper_daily_stop_state(self, reason_label, reference_time=None):
        ...
        if self.paper_daily_stop_day_key != day_key:
            logger.info(
                "PAPER SYMBOL DAILY SL STOP RESET | "
    ...
    def activate_paper_symbol_daily_sl_stop(self, symbol, trade_id, exit_time=None):
        day_key = self.refresh_paper_daily_stop_state("PAPER_SYMBOL_SL_TRIGGER", exit_time)
        ...
        if not stop or not stop.get("active"):
            ...
            logger.warning(
                "PAPER SYMBOL DAILY SL STOP ACTIVATED | "
                f"stage={BOT_STAGE} "
                f"day_key={day_key} "
                f"symbol={symbol} "
                f"trigger_trade_id={trade_id} "
            ...
            logger.warning(
                "PAPER SYMBOL DAILY SL STOP ALREADY ACTIVE | "
                f"stage={BOT_STAGE} "
                f"day_key={day_key} "
                f"symbol={symbol} "
                f"trigger_trade_id={trade_id} "
    ...
    def evaluate_paper_execution_risk_regime(self, job):
        ...
        if stage != "PAPER":
            return {
                "stage": stage,
                "risk_branch": "non_paper_regime",
                "execution_allowed": True,
                "reason": "paper_symbol_daily_sl_stop_not_applicable",
            }

        day_key = self.refresh_paper_daily_stop_state("PAPER_EXECUTION_GATE")
        ...
        if stop and stop.get("active") and stop.get("day_key") == day_key:
            reason = "paper_symbol_daily_sl_stop_active"
            logger.warning(
                "PAPER SYMBOL DAILY SL STOP DENIAL | "
                f"stage={stage} "
                f"day_key={day_key} "
            ...
            return {
                "stage": stage,
                "risk_branch": "paper_symbol_daily_sl_stop_active",
                "execution_allowed": False,
                "day_key": day_key,
        ...
        logger.info(
            "PAPER SYMBOL DAILY SL STOP CLEAR | "
            f"stage={stage} "
            f"day_key={day_key} "
            f"incoming_symbol={incoming_symbol} "
            "trigger_trade_id=None "
            "reason=paper_symbol_daily_sl_stop_clear"
        )
        return {
            "stage": stage,
            "risk_branch": "paper_symbol_daily_sl_stop_clear",
            "execution_allowed": True,
            "day_key": day_key,
            "trigger_trade_id": None,
            "reason": "paper_symbol_daily_sl_stop_clear",
    ...
    def build_in_session_broker_fill_reconciliation_candidate(self, record):
        ...
        try:
            ...
            logger.exception(
                ...
                f"trade_id={trade_id} symbol={record.get('symbol')} reason=fills_unavailable"
            )
            return {
                "ok": False,
        ...
        match_sources = {"order_id": 0, "perm_id": 0}
        matched_count = 0
        ...
        for fill in fills:
            ...
            leg, match_source = self.resolve_reconstructed_fill_leg(record, execution)
            if leg is None:
                continue
        ...
        if matched_count <= 0:
            logger.warning(
                "BROKER_FILL_RECONCILIATION_AMBIGUOUS | "
                f"trade_id={trade_id} symbol={record.get('symbol')} reason=no_exact_matching_fills"
            )
            return {
                "ok": False,
    ...
    def apply_in_session_broker_fill_reconciliation(self, trade_id, candidate, reason_label):
        with self.trade_analysis_lock:
            record = self.trade_analysis.get(trade_id)
    ...
    def reconcile_trade_from_in_session_broker_fills(self, trade_id, reason_label):
        ...
        if not candidate.get("ok"):
            logger.warning(
                ...
                f"reason={candidate.get('reason') or 'candidate_unavailable'}"
            )
            return False
        ...
        if not valid:
            logger.warning(
                "BROKER_FILL_RECONCILIATION_AMBIGUOUS | "
                f"trade_id={trade_id} symbol={record_snapshot.get('symbol')} "
                f"reason={validation_reason}"
            )
            logger.warning(
                "BROKER FILL RECONCILIATION SKIPPED | "
    ...
    def finalize_trade_if_complete(self, trade_id):
        with self.trade_analysis_lock:
            ...
            if exit_complete:
                ...
                if record.get("bot_stage") == "PAPER" and paper_sl_component_present:
    ...
    def handle_connectivity_error(self, error_code, error_string):
        ...
        if error_code == 1102:
            ...
            if self.connectivity_reconciliation_required:
                ...
                try:
                    self.reconcile_active_trades_from_in_session_broker_fills(
    ...
    def handle_webhook_signal(self, data):
        ...
        if self.is_runtime_symbol_disabled(normalized["symbol"]):
            ...
            return "symbol_runtime_disabled"
                ...
                with self.execution_lock:
                    ...
                    if not paper_risk_regime["execution_allowed"]:
                        ...
                        logger.warning(
                            "EXECUTION WORKER EXECUTION SKIPPED | "
                            f"symbol={job.get('symbol')} "
                            "reason=paper_symbol_daily_sl_stop"
                        )
                        continue
